...
void codemakerFeedback(){
    int i,j;
    b = 0; w = 0;
    int temp[]= {0,0,0,0}; // to indicate which position is already looked up
    for(i=0;i<4;i++){
    ...
    for(i=0; i<4;i++){
        ...
        for(j=0;j<4;j++){
            if(i!=j && temp[j] == 0 && guess[i] == hidden_code[j]){ //because for whites we need not check in the same position
                w++; temp[j] = 1;
                break;
            }
        }
...
void addConstraints(){
    ...
    vector<int> clause;
    if(b == 0){
        if(w==0){ //reject all chosen colours
            for(i=0;i<4;i++){
                for(k=0; k<4;k++){
            ...
            for(i=0;i<4;i++){
                ...
                for(j=0;j<4;j++){
                    if(i!=j && guess[i]-1 != guess[j]-1){ //here one extra check is required  if both of them are the same colour then that position is also not allowed
                        clause.push_back(color_with_position[guess[i]-1][j]);
                    }
                }
                vec.insert(clause);
            ...
            for(i=0;i<4;i++){ //if the colour is present then it is in the wrong position
                ...
                clause.push_back(-color_with_position[guess[i]-1][i]); 
                for(j=0;j<4;j++){
                    if(i!=j && guess[i]-1 != guess[j]-1) //similarly here also one extra check is required.
                        clause.push_back(color_with_position[guess[i]-1][j]); 
                }
                vec.insert(clause);
            }
    ...
    if(b == 1){
        ...
        //atmost one of them is present in the correct position
        for(i=0;i<4;i++){
            for(j=i+1; j<4;j++){
                clause.push_back(-color_with_position[guess[i]-1][i]); 
...
void get_hidden_code_with_duplicates(){
    int i=0,j,x;
    srand(time(0));
    while(i < 4){
        x=rand()%8;
...
int main(void)
{
    ...
    get_hidden_code_with_duplicates();
    //hidden_code[0] = 1; hidden_code[1] = 1; hidden_code[2] = 3; hidden_code[3] = 8; 
